...
class DBManager {
    ...
    public function init($host = NULL, $username = NULL, $password = NULL, $db = NULL, $port = NULL, $prefix = '') {
        \WEPPO\Ressource\TableRecord::initDB($host, $username, $password, $db, $port);
        \WEPPO\Ressource\TableRecord::setPrefix($prefix);
 ...
 #       //SET sql_mode = 'NO_AUTO_VALUE_ON_ZERO';
    ...
    public function query($sql) {
        $sql = str_replace('%%', \WEPPO\Ressource\TableRecord::getPrefix(), $sql);
        try {
            $res = \WEPPO\Ressource\TableRecord::rawQuery($sql);
        } catch (\Exception $e) {
            $this->error = $e->getMessage();
    ...
    static public function col(string $name, $type, $size=null, $options=null, $default=null) {
        ...
        if ($size === null && $options === null) {
            ...
            $type = str_replace('%%', \WEPPO\Ressource\TableRecord::getPrefix(), $type);
            return $name.' '.$type;
        }
