...
class TableRecord extends \stdclass {
    ...
    /**
     * 'fieldname' => 'cast_id',
     * 'fieldname' => array('load'=>callback, 'save'=>callback)
     */
    static function getCast() {
        return array();
    }

    static $resolveForeinFields = array();
    protected static $doResolve = true;
    ...
    public function assign(array $data) {
        ...
        foreach ($data as $k => $v) {
            if (isset($cast[$k])) {
                ...
                } else if (isset($c['load']) && is_callable($c['load'])) {
                    $v = call_user_func_array($c['load'], array($v, &$this));
                } else {
                    switch ($c) {
                        case 'json':
                            $v = json_decode($v);
    ...
    static public function doResolve($r = true) {
        static::$doResolve = $r;
    }

    public function resolve($k) {
        if (!isset(static::$resolveForeinFields[$k]))
            return false;
        $v = &static::$resolveForeinFields[$k];

        if (!isset($v['foreinKeyField']) || !isset($v['class']))
            return false;

        $class = $v['class'];
        $class::where('id', $this->{$v['foreinKeyField']});
        $a = $class::getOne();

        // falls nichts vorhanden - leeres objekt laden
        if (!is_object($a)) {
            $a = new $class();
            $a->loadEmpty();
        }
        ...
        if (isset($v['valueField'])) {
            if (is_object($a) && isset($a->{$v['valueField']})) {
                $this->{$k} = $a->{$v['valueField']};
            } else {
                $this->{$k} = null;
            }
        } else {
    ...
    public function resolveForeinFields() {
        //_o(static::$resolveForeinFields, static::getStaticClass());
        foreach (static::$resolveForeinFields as $k => &$v) {
            if (!$this->resolve($k))
                continue;
        }
    }
