...
class Image extends AbstractElement
{
    ...
    public function getImageStringData($base64 = false)
    {
        ...
        if ($this->sourceType == self::SOURCE_GD) {
            ## MOD Stefan Beyer 23.05.2016 to provide GD-Recources as source
            if (is_resource($actualSource)) {
                $imageResource = $actualSource;
            } else {
                trigger_error('BAD');
    ...
    private function checkImage($source)
    {
        ...
        if ($this->sourceType == self::SOURCE_ARCHIVE) {
            ...
            if (is_resource($source)) {
                $imageData = array(@imagesx($source), @imagesy($source), IMAGETYPE_PNG);
            } else {
    ...
    private function setFunctions()
    {
        ...
        if (is_resource($this->source)) {
            $this->imageCreateFunc = function ($s) {return $s;};
        }
    }
